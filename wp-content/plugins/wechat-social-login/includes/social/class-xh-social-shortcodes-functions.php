...
if(!function_exists('xh_social_login_wide')){
	function xh_social_login_wide($echo =true){
		ob_start();
		require XH_Social::instance()->WP->get_template(XH_SOCIAL_DIR, 'account/login-wide.php');
		
		if($echo) echo ob_get_clean(); else return ob_get_clean();
	}
}

if(!function_exists('xh_social_login_short')){
	function xh_social_login_short($echo =true){
		ob_start();
		require XH_Social::instance()->WP->get_template(XH_SOCIAL_DIR, 'account/login-short.php');
